...
def reset_timer():
    ...
    tick_label.config(text="Timer", fg=GREEN)
    reps = 0
...
def start_timer():
    ...
    if reps % 8 == 0:
        ...
        tick_label.config(text="Break", fg=RED)
    elif reps % 2 == 0:
        count_down(short_break_sec)
        tick_label.config(text="Break", fg=PINK)
    else:
        count_down(work_sec)
        tick_label.config(text="Work", fg=GREEN)
...
tick_label = Label()
tick_label.config(text="Timer", bg=YELLOW, fg=GREEN, font=(FONT_NAME, 45, "bold"))
tick_label.grid(row=1, column=2)
